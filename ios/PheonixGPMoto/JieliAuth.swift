...
enum JieliAuth {
    ...
    private static func keySchedule(_ input: [UInt8]) -> [UInt8] {
        ...
        for round in 0 ..< 16 {
            for index in buffer.indices {
                buffer[index] = (buffer[index] << 3) | (buffer[index] >> 5)
            }
    ...
    private static func substitute(_ state: [UInt8]) -> [UInt8] {
        ...
        for index in [0, 3, 4, 7, 8, 11, 12, 15] {
            result[index] = JieliTables.sbox[Int(result[index])]
        }
        for index in [1, 2, 5, 6, 9, 10, 13, 14] {
            result[index] = JieliTables.inverseSbox[Int(result[index])]
        }
        return result
    }
    ...
    private static func fibonacciMix(_ s: [UInt8]) -> [UInt8] {
        ...
        func twice(_ value: UInt8) -> UInt8 {
            value &* 2
        }
